...
export const getApiBaseUrl = () => {
    ...
    if (isBrowser() && !window.location.hostname.includes("localhost") && baseUrl.includes("localhost")) {
...
export const getSocketBaseUrl = () => {
    const configured = process.env.NEXT_PUBLIC_SOCKET_URL || process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000";
    let baseUrl = configured.replace(/\/+$/, "").replace(/\/api$/, "");
...
export async function apiRequest(endpoint, options = {}) {
    ...
    try {
        ...
        const message = `Backend is not reachable. Check API URL or server status. (${normalizedEndpoint})`;
        if (isDevelopment) console.error(`[Storvia API] Network error for ${normalizedEndpoint}:`, error);
