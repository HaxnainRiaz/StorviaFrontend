...
const COMMERCE_VIEWS = new Set([
    'products', 'cart', 'checkout', 'product', 'collection', 'order-tracking',
]);

export function shouldRenderFullImportedPage(view, page) {
    if (!page) return false;
    // Commerce routes always use Storvia dynamic body with imported chrome
    if (COMMERCE_VIEWS.has(view)) return false;
    // Home, contact, and custom pages keep imported design shell; dynamic sections still render via binding types
    if (['home', 'contact', 'page'].includes(view)) return true;
    return false;
}
