...
export function resolvePageForView(view, pageSlug, pages = [], productSlug = '') {
    if (!pages.length) return null;

    if (view === 'product' && productSlug) {
        const base = String(productSlug).replace(/-\d+$/, '').toLowerCase();
        const slug = String(productSlug).toLowerCase();
        const matched =
            pages.find(
                (p) =>
                    String(p.slug || '').toLowerCase() === slug ||
                    String(p.slug || '').toLowerCase() === base ||
                    String(p.id || '').toLowerCase() === slug ||
                    String(p.id || '').toLowerCase() === base ||
                    String(p.fileName || '')
                        .replace(/\.html$/i, '')
                        .split('/')
                        .pop()
            ...
            pages.find((p) => p.sections?.some((s) => /product-page|class=["']product-page/i.test(s.html || '')));
    ...
    if (view === 'product') {
        return pages.find((p) => p.type === 'product')
            || matchPagePattern(pages, /^(product|product-detail|single-product)(\.html)?$/i)
            || pages.find((p) => ['product', 'product-detail'].includes(p.slug))
            || pages.find((p) => p.sections?.some((s) => /product-page|class=["']product-page/i.test(s.html || '')));
    }

    if (view === 'collection') {
        return pages.find((p) => p.type === 'collection') || matchPagePattern(pages, /^collection/i);
    }

    if (view === 'order-tracking') {
        return pages.find((p) => p.type === 'track_order') || matchPagePattern(pages, /track/i);
    }
    ...
    if (view === 'page' && pageSlug) {
        ...
        if (slug === 'wishlist') {
            return (
                pages.find((p) => p.type === 'wishlist') ||
                matchPagePattern(pages, /^wishlist/i) ||
                pages.find((p) => String(p.slug || '').toLowerCase() === 'wishlist')
            );
        }
...
/** Use the full imported HTML page whenever the design provides one for this view. */
export function shouldRenderFullImportedPage(view, page) {
    return Boolean(page?.sections?.length);
}
